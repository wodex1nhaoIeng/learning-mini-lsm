...
impl MemTable {
    ...
    pub fn put(&self, _key: &[u8], _value: &[u8]) -> Result<()> {
        ...
        self.approximate_size
            .fetch_add(key_len + value_len, std::sync::atomic::Ordering::Relaxed);
    ...
    pub fn scan(&self, _lower: Bound<&[u8]>, _upper: Bound<&[u8]>) -> MemTableIterator {
        let range = (map_bound(_lower), map_bound(_upper));
...
impl MemTableIterator {
    /// Convert a skipmap entry to a key-value pair.
    pub fn entry_to_item(entry: Option<Entry<'_, Bytes, Bytes>>) -> (Bytes, Bytes) {
        match entry {
            Some(x) => (x.key().clone(), x.value().clone()),
